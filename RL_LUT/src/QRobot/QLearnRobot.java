...
public class QLearnRobot extends AdvancedRobot 
{
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		QuanDistance = QuantifyDistance(distance);
			...
			for(int i = 0; i < nActions; i++)
			{
				...
				QValues[i] = LUT.outputFor(stateAction);
			}
	...
	public int QuantifyDistance (double d) {
	...
	public int QuantifyHeading (double h) {
		
		if(Math.ceil(h/45)==0)
			return 1;
		else
			return (int) Math.ceil(h/45);	
		
		
	}
	
public int QuantifyBearing (double b) {
	
	if(Math.ceil(b/45)+4==0)
		return 1;
	else
		return (int) (Math.ceil(b/45)+4);
...
public int QuantifyX (double x) {

	if(x > getBattleFieldWidth()*0.9)
		return 3;
	else if(x < getBattleFieldWidth()*0.1)
		return 1;
	else
		return 2;	
